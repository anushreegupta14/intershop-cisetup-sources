...
public class PunchoutSpec extends GebReportingSpec implements Authentication, OCIUserLogin,TestDataUsage
{
    ...
    def "Check multiple logins will each get their own baskets"()
    {
        ...
        when: "Navigating to a product's detail page"
    ...
    def "Check the OCI Punchout catalog transfer"()
    {
        ...
        when: "Navigating to a product's detail page"
            to ProductDetailPage, SKU:ociProductWithWarrantyId
    ...
    def "Check limited storefront visibility of OCI user"()
    {
        ...
        when: "Navigating to a product's detail page"
            to ProductDetailPage, SKU:DemoProduct.SONY_VLP_SW225.product.id
	...
	def "Create a Punchout Configuration in My Account"()
    {
        ...
        when: "Click the configuration button to open the punchout configuration page"
            configurationLink.click()

		then: 
            at PunchoutConfigurationPage
            
            configurationItems[2].transform.value "AA_{Sku}"
            configurationItems[2].mappingfrom.value "asdf"
            //configurationItems[2].mappingto.value "1234"
        ...
        then: "Error, missing to value"
            assert $(".mapping-row-NEW_ITEM-MATGROUP .punchout-item-mapping-alert").first().text() == "The mapping to value is missing."
             
        when: "set custom values for fields, no from value"   
            configurationItems[2].mappingfrom.value ""
            configurationItems[2].mappingto.value "1234"
       
        and: "I save the configuration"
            saveButton.click()
            
        then: "Error, missing from value"
            assert $(".mapping-row-NEW_ITEM-MATGROUP .punchout-item-mapping-alert").first().text() == "The mapping from value is missing."
        
        when: "set custom values for fields, no parameter value"
        ...
        then: "Error, missing parameter value"
            assert $(".punchout-item-value-alert")[2].text() == "The parameter value is missing."
        
        when: "set custom values for fields"
            configurationItems[2].transform.value "AA_{Sku}"
            
        and: "Add row"
            $(".add-more-rows-NEW_ITEM-MATGROUP").find("a").click()
            waitFor{ $(".mapping-row-NEW_ITEM-MATGROUP .mapping-from-container").find("input")[1].size()>0}
            $(".mapping-row-NEW_ITEM-MATGROUP .mapping-from-container").find("input")[1].value "jklo"
            $(".mapping-row-NEW_ITEM-MATGROUP .mapping-to-container").find("input")[1].value "7890"
        ...
        when: "Click the configuration link again"
            configurationLink.click()
        ...
        then: 
            ...
            assert configurationItems[2].mappingfrom[0].value() == "asdf"
            assert configurationItems[2].mappingto[0].value() == "1234"
            assert configurationItems[2].format.value() == "UpperCase"
            assert $(".mapping-row-NEW_ITEM-MATGROUP .mapping-from-container").find("input")[1].value() == "jklo"
            assert $(".mapping-row-NEW_ITEM-MATGROUP .mapping-to-container").find("input")[1].value() == "7890"
        ...
        when: "I reset custom values for fields"
            configurationItems[2].transform.value ""
            configurationItems[2].mappingfrom.value ""
            configurationItems[2].mappingto.value ""
        ...
        then: "My modifications arent displayed anymore"
            at PunchoutConfigurationPage
        	
            assert configurationItems[2].label == "NEW_ITEM-MATGROUP"
            assert configurationItems[2].transform.value() == ""
            assert configurationItems[2].mappingfrom.value() == ""
            assert configurationItems[2].mappingto.value() == ""
            assert configurationItems[2].format.value() == ""
            assert $(".mapping-row-NEW_ITEM-MATGROUP .mapping-from-container").size() == 2
